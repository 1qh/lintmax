...
const createStepExecutor = ({
  ...
  const runCompactContinue = async ({ mode }: { mode: 'check' | 'fix' }) => {
    try {
      await runCompact({
        env,
        mode,
...
const runLint = async ({ command }: { command: 'check' | 'fix' }) => {
  ...
  const { clearFailures, runCompactContinue, runSteps, throwIfFailures } = createStepExecutor({
    env,
    failures,
    root: cwd
  })
