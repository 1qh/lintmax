...
const decoder = new TextDecoder(),
  cacheDir = 'node_modules/.cache/lintmax',
  cwd = process.cwd(),
  ...
  parseJsonObject = ({ text }: { text: string }): Record<string, unknown> => {
    const parsed: unknown = JSON.parse(text)
    return isRecord(parsed) ? parsed : {}
  },
  decodeText = (bytes: Uint8Array | undefined) => decoder.decode(bytes ?? new Uint8Array()),
  pathExists = async ({ path }: { path: string }): Promise<boolean> => file(path).exists(),
