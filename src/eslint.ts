...
interface LintmaxOptions {
  ...
  tailwind?: string | false
  tsconfigRootDir: string
  turbo?: boolean
}

const createBaseConfig = (options: LintmaxOptions): ReturnType<typeof defineConfig> => {
  const gitignorePath = join(options.tsconfigRootDir, '.gitignore')
...
const createReactConfig = (options: LintmaxOptions): ReturnType<typeof defineConfig> => {
  const tailwindSettings: Record<string, unknown> = {}
  if (typeof options.tailwind === 'string')
    tailwindSettings['better-tailwindcss'] = {
      entryPoint: options.tailwind
    }

  return defineConfig(reactHooks.configs.flat['recommended-latest'] as { rules: Linter.RulesRecord }, {
    files: ['**/*.ts', '**/*.tsx'],
    ...
    rules: {
      ...
      ...(typeof options.tailwind === 'string' ? eslintPluginBetterTailwindcss.configs['recommended-error'].rules : {}),
...
const createNextjsConfig = (): ReturnType<typeof defineConfig> =>
  defineConfig({
    files: ['**/*.ts', '**/*.tsx'],
    plugins: {
...
const eslintFactory = (options: LintmaxOptions): ReturnType<typeof defineConfig> => {
  ...
  if (options.nextjs !== false)
    configs.push(...createNextjsConfig())
